...
- Application to be deployed on Cloud server: GCP virtual machines.
...
It would firstly generating a random driver and random car. Each driver and car are given a certain ratings. This is where service 4 (rating-api), will work out the total ratings for those 2 randomly generated information. The maximum ratings that can be achieved is 250. 

`Service 1`
> This will interact and provide rating output to the user. The rating output will be acquired from the information on Service 4. 
...
`Service 4`
> Take in the output from Service 2 and 3 and calculate their ratings based on the car and driver that was generated.
...
Once I was commited to my idea, I started to work on the design of the application that it'd like the front end user to see. I decided to go with a very simple design for the front end. 
...
MoSCoW prioritisation was used and this dictated the tasks that were put into the sprint backlog. A visual of my Trello board can be seen below to help understand this. 
...
This particular task can be taken to the project backlog as part of future improvements.
...
- The user will interact with the front-end via the Nginx reverse proxy service. The front-end will display the information via HTML. The Nginx reverse proxy will be set to listen on port 80. 
...
- The swarm-worker VM will also be configured via an ansible playbook. The swarm-worker will be joined to the swarm-manager via the ansible playbook so that swarm is ready before I deploy my application. The swarm-manager will dictate the containers that the swarm-worker will run.
...
`Project Tracking`
> As mentioned previously a Trello Board used for my project tracking software. I used this to organise my workload and set myself goals to deliver the application by deadline.

`Version Control System`
> Git was used as the version control for my project.
> GitHub was used as my version control system repositry.
> Several feature branches were used to ensure that a stable version of the application is available for use.
...
> A GitHub webhook was integrated with Jenkins. This allows Jenkins to automatically test, build, push docker images, and deploy my application anytime I make changes to it.
> Separate tests were created for all services except Nginx service. In order to make ease of automation a script called `test.sh` was written so that each service is tested reccursively.

`Containerisation`
> Docker was used as my containerisation tool. There are many advantages to containerating my application. Using containers can create predictable environments that are isolated from other apps. We can have a cost-effective and a speedy deployment. I can also roll back to a previous of my application by using a different image. 
...
A GitHub webhook was integrated with Jenkins. This meant that everytime I made changes up to GitHub, the webhook would trigger Jenkins to do the following:

- Run tests, using the pytest module. 
- Run ansible playbook to install my dependencies onto the deployment VMs and initialise a swarm.
...
The first build was made completely fresh where ansible was used to install my dependencies on the 3 VM's used for deployment. You can view this log by clicking [here](https://github.com/Jamalh8/QA-Practical-Project/blob/feature/ansible/images_and_diagram/Jenkins-logs-fresh-build.txt)

I ran this pipeline again to show that the dependencies were infact successful and did a re-deployment of my application. This log can be viewed by clicking [here](https://github.com/Jamalh8/QA-Practical-Project/blob/feature/ansible/images_and_diagram/Jenkins-logs-second-build.txt)
...
I had several playbook failures during this project. However, it was a great learning process as it helped me understand the issues and research how I can correct these.
...
The most difficult part of the pipeline for me was to implement Ansible, build and push new images to dockerhub, and deploy to the swarm. I stayed resilient and kept trying. In the end I was rewarded with a working pipeline that automated my testing, infastructure and dependencies installation, building and pushing images to dockerhub, and deploying the application. 
...
Most notably, the difficult part of this section was to obtain the random information from service 2 and 3 and use service 4 to provide a logical output. 

### Future Improvements
___

I currently have a few things in mind that I'd like to implement into my project for future improvements.
...
This project has given me vast progess in my tech career. I have further enhanced my tech knowledge but also the tools at my disposal. 
